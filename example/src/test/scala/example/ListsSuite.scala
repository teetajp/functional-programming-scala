...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of list with negative numbers") {
    assertEquals(sum(List(-3, 0, 8, -10)), -5)
  }

  test("sum of list with only zeroes") {
  ...
  test("max throws NoSuchElementException when given empty list") {
    try
      max(List())
      fail("No exception has been thrown")
    catch
  ...
  test("max of list with negative elements") {
    assertEquals(max(List(-9, 2, -2)), 2)
  }

  test("max of list with repeated elements") {
    assertEquals(max(List(7, 7, 7)), 7)
  }

  test("max of list with zeros") {
    assertEquals(max(List(0, 0)), 0)
  }
